...
case class ShiftStartLogEntry(timestamp: LocalDateTime, hour: Int, minute: Int, guard: Int) extends LogEntry
case class SleepLogEntry(timestamp: LocalDateTime, hour: Int, minute: Int) extends LogEntry
case class WakeLogEntry(timestamp: LocalDateTime, hour: Int, minute: Int) extends LogEntry
case class NullLogEntry() extends LogEntry {
  val guard = -1
  val timestamp = LocalDateTime.MIN
  val hour = 0
  val minute = 0
}
...
trait Data {
  ...
  lazy val ShiftStartEntryExtractor = """\[(\d{4}\-\d{2}\-\d{2}) (\d{2}):(\d{2})\] Guard #(\d+) begins shift""".r
  lazy val SleepEntryExtractor = """\[(\d{4}\-\d{2}\-\d{2}) (\d{2}):(\d{2})\] falls asleep""".r
  lazy val WakeEntryExtractor = """\[(\d{4}\-\d{2}\-\d{2}) (\d{2}):(\d{2})\] wakes up""".r

  lazy val logEntries: Seq[LogEntry] = {
    fullData.map {
//    testData.map {
      entry =>
        entry match {
          case ShiftStartEntryExtractor(date, hour, minute, guard) =>
            ShiftStartLogEntry(LocalDateTime.parse(s"${date}T${hour}:${minute}"), hour.toInt, minute.toInt, guard.toInt)

          case SleepEntryExtractor(date, hour, minute) =>
            SleepLogEntry(LocalDateTime.parse(s"${date}T${hour}:${minute}"), hour.toInt, minute.toInt)

          case WakeEntryExtractor(date, hour, minute) =>
            WakeLogEntry(LocalDateTime.parse(s"${date}T${hour}:${minute}"), hour.toInt, minute.toInt)

          case _ =>
            println(s"Unparseable: $entry")
            NullLogEntry()
        }
    }.sortBy(_.timestamp)
  }
